...
#ifdef PIC
#define DEBUG_PIC(msg, ...) sprintf(print_buf, msg, ##__VA_ARGS__); lsc_uart_tx(print_buf)
#else
#define DEBUG_PIC(msg, ...)
#endif
...
unsigned char SPI_Interrupt_Available(void)
{
	DEBUG_PIC("pic_dev->pic_en = 0x%04x\r\n", pic_dev->pic_en);
  return(pic_dev->pic_en&(1<<SPISR_IRQ1));
}
